...
import { FaFire, FaTrophy, FaUserGraduate, FaSignOutAlt, FaBookOpen, FaNetworkWired, FaMapMarkerAlt } from 'react-icons/fa';
        ...
        <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex">
            ...
            <aside className="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col p-6 sticky top-0 h-screen shadow-sm z-20">
                <div className="flex items-center gap-3 mb-10 text-net-blue">
                    ...
                    <span className="text-2xl font-bold tracking-tighter">NetLearn</span>
                </div>
                <nav className="space-y-2 flex-1">
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition cursor-pointer">
                        <FaTrophy /> Leaderboard
                    </div>
                </nav>
                <Link href={route('logout')} method="post" as="button" className="flex items-center gap-2 text-slate-400 hover:text-red-500 transition mt-auto font-medium">
            ...
            <main className="flex-1 relative overflow-y-auto h-screen">
                ...
                <div className="md:hidden p-4 bg-white/80 backdrop-blur sticky top-0 z-50 flex justify-between items-center border-b border-slate-200">
                    ...
                    <span className="text-orange-500 flex items-center gap-1 font-bold"><FaFire /> {gamification.streak}</span>
                </div>
                ...
                <div className="py-12 px-4 flex flex-col items-center min-h-full relative z-10">
                    ...
                    <div className="text-center mb-16">
                        <h1 className="text-3xl font-black text-slate-800 mb-2">Jalur Kompetensi</h1>
                    ...
                    <div className="flex flex-col items-center w-full max-w-md">
                        ...
                        {weeks.map((week, weekIndex) => (
                            <div key={week.id} className="w-full flex flex-col items-center">
                                ...
                                <div className="flex flex-col items-center">
                                    {week.lessons.map((lesson, index) => {
                                        ...
                                        const isVeryLast = isLastInWeek && weekIndex === weeks.length - 1;
                        ...
                        <div className="mt-8 mb-24 flex flex-col items-center opacity-50 grayscale">
                            <div className="w-24 h-24 bg-slate-200 rounded-full flex items-center justify-center border-4 border-slate-300">
                                <FaTrophy size={40} className="text-slate-400" />
                            </div>
                            <span className="font-bold text-slate-400 mt-2">SERTIFIKAT</span>
                        </div>
            ...
            <aside className="w-80 bg-white p-6 hidden lg:block sticky top-0 h-screen border-l border-slate-200 shadow-sm z-20">
                <div className="flex justify-between mb-8 bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <div className="flex items-center gap-2 text-orange-500 font-bold">
                        <FaFire /> <span>{gamification.streak} Hari</span>
                    </div>
                ...
                <div className="border border-slate-200 rounded-2xl p-6 mb-6 relative overflow-hidden">
                    <h3 className="font-bold text-lg mb-2 text-slate-700">Target Hari Ini 🎯</h3>
                    <p className="text-slate-400 text-xs mb-4">Selesaikan 2 Node untuk menjaga streak!</p>
                ...
                <div className="bg-net-blue text-white rounded-2xl p-6 shadow-lg shadow-blue-200 relative">
                    ...
                    <p className="text-blue-100 text-xs mb-4">{auth.user.email}</p>
                    <div className="text-2xl font-black">{gamification.rank}</div>
                </div>
            </aside>
        </div>
    );
